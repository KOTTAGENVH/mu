...
import { updateWishlist } from "@/app/api/client/services/list/api";
...
export default function EditWishListModal({
  ...
  const handleEdit = async () => {
    try {
      //Validate the form
      if (newName === "") {
        alert("Wish list Name is required");
        return;
      }
      setIsLoaded(true);

      const response = await updateWishlist(id, newName);
      ...
      if (data.success) {
        ...
        if (data.message) {
          ...
          alert("Error editing wish list");
        }
        setIsLoaded(false);
        // console.error("Error editing wish list :", data.message);
      }
    } catch (error) {
      alert(error instanceof Error ? error.message : "Error editing wish list");
      setIsLoaded(false);
            ...
            >
              <div className="relative bg-white/10 dark:bg-black/10 backdrop-blur-xl rounded-3xl border border-white/20 dark:border-white/10 shadow-2xl overflow-hidden">
                ...
                >
                  ...
                  >
                    {isLoaded ? "Editing..." : "Edit Wish List"}
                  </button>
