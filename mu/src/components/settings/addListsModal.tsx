...
import { addWishlist } from "@/app/api/client/services/list/api";
...
export default function AddWishListModal({
  ...
  const handleAdd = async () => {
    try {
      ...
      const response = await addWishlist(newName);
      ...
      if (data.success) {
        ...
        setIsLoaded(false);
        // console.error("Error adding list :", data.message);
      }
    } catch (error) {
      alert(error instanceof Error ? error.message : "Error adding list");
      setIsLoaded(false);
